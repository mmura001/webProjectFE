...
export const homeSlice = createSlice({
    ...
    reducers:{
        ...
        setMe:(state,action) => {
            console.log("me fun", state)
            state.me = action.payload;
        },
        setTokenFunc:(state,action) => {
            console.log("inside", action.payload, state);
...
export const TokenSelector = (state) => state.homeSlice;
...
export function getUserInfo(){
    ...
    return async (dispatch) => {
        try{
            dispatch(setMe({}));
            Api.call("/admin/myLipClaims",{method:'GET'},(response)=> {
                //if success
...
export function registerFunction(form){
    ...
    return async (dispatch) => {
        try{
            Api.call("/auth/register",{method:'POST', body: form},(response)=> {
                //if success
                response = JSON.parse(response);
...
export function loginFunction(form){
    ...
    return async (dispatch) => {
        try{
            Api.call("/auth/login",{method:'POST', body: form},(response)=> {
                //if success
                response = JSON.parse(response);
                localStorage.setItem('token', true)
...
export function updateFunction(form){
    ...
    return async (dispatch) => {
        try{
            Api.call("/auth/profileusername",{method:'PUT', body: form},(response)=> {
                //if success
                response = JSON.parse(response);
                localStorage.setItem('token', true)
                localStorage.setItem('email', response['email'])
...
export function updatePasswordFunction(form){
    ...
    return async (dispatch) => {
        try{
            Api.call("/auth/profilepassword",{method:'PUT', body: form},(response)=> {
                //if success
                response = JSON.parse(response);
                localStorage.setItem('token', true)
                localStorage.setItem('email', response['email'])
